...
# Log stripping/formatting
# Regex pattern to strip from log lines (e.g., remove Dapr/NX prefixes)
# Example: STRIP_PATTERN='^[a-z]+: == APP == [0-9-]+ [0-9:,]+ - [A-Za-z]+ - (INFO|DEBUG|WARNING|ERROR) - '
STRIP_PATTERN="${STRIP_PATTERN:-}"
# Common presets (set STRIP_PRESET in .nxlogsrc)
# dapr: Strips 'appname: == APP == timestamp - Logger - LEVEL - ' prefix
# nx: Strips '[appname] ' prefix
STRIP_PRESET="${STRIP_PRESET:-}"
# Strip N characters from front of each line
STRIP_FRONT="${STRIP_FRONT:-0}"
# Strip N characters from back of each line  
STRIP_BACK="${STRIP_BACK:-0}"
# Strip text before a delimiter (e.g., ' - ' to remove everything before first ' - ')
STRIP_BEFORE="${STRIP_BEFORE:-}"
# Strip text after a delimiter (e.g., ' | ' to remove everything after first ' | ')
...
discover_apps_from_nx() {
    ...
    # Method 1: Try 'nx show projects' (Modern NX)
    # We try to filter for apps if possible, but 'show projects' lists all.
    # 'nx show projects --type app' is supported in newer versions.
    if $nx_cmd show projects --help &> /dev/null; then
        local nx_output
        if nx_output=$($nx_cmd show projects --type app 2>/dev/null); then
             while IFS= read -r app; do
                [[ -n "$app" ]] && apps+=("$app")
             done <<< "$nx_output"
        elif nx_output=$($nx_cmd show projects 2>/dev/null); then
             # If --type app fails, return all (might include libs, but better than nothing)
             while IFS= read -r app; do
                [[ -n "$app" ]] && apps+=("$app")
             done <<< "$nx_output"
        fi
    ...
    # Method 2: Try to get apps from nx.json (Legacy/Simple)
    if [[ ${#apps[@]} -eq 0 ]] && [[ -f "$project_root/nx.json" ]] && command -v jq &> /dev/null; then
...
rotate_log_file() {
    ...
    # Rotate if lines >= max_lines (not just greater than)
...
# Build a filter command for stripping log prefixes (for use in pipelines)
# Returns a command string that can be used with eval or in a pipeline
build_strip_filter() {
    local filter="cat"
    
    # Apply preset patterns
    case "$STRIP_PRESET" in
        dapr)
            filter="sed -E 's/^[a-zA-Z0-9_-]+: == APP == [0-9]{4}-[0-9]{2}-[0-9]{2}[T ][0-9]{2}:[0-9]{2}:[0-9]{2}[,.][0-9]+(Z)? - [A-Za-z._]+ - (INFO|DEBUG|WARNING|ERROR|CRITICAL) - //'"
            ;;
        nx)
            filter="sed -E 's/^\[[a-zA-Z0-9_-]+\] //'"
            ;;
        minimal)
            filter="sed -E 's/^[0-9]{4}-[0-9]{2}-[0-9]{2}[T ][0-9]{2}:[0-9]{2}:[0-9]{2}[,.][0-9]+(Z)? - [A-Za-z._]+ - (INFO|DEBUG|WARNING|ERROR|CRITICAL) - //'"
            ;;
    esac
    
    # Apply custom strip pattern if set (chain with previous filter)
    if [[ -n "$STRIP_PATTERN" ]]; then
        if [[ "$filter" == "cat" ]]; then
            filter="sed -E 's/$STRIP_PATTERN//'"
        else
            filter="$filter | sed -E 's/$STRIP_PATTERN//'"
        fi
    fi
    
    # Strip N characters from front using cut
    if [[ "$STRIP_FRONT" -gt 0 ]] 2>/dev/null; then
        local start=$((STRIP_FRONT + 1))
        if [[ "$filter" == "cat" ]]; then
            filter="cut -c${start}-"
        else
            filter="$filter | cut -c${start}-"
        fi
    fi
    
    # Strip N characters from back using sed
    if [[ "$STRIP_BACK" -gt 0 ]] 2>/dev/null; then
        if [[ "$filter" == "cat" ]]; then
            filter="sed 's/.\{$STRIP_BACK\}$//'"
        else
            filter="$filter | sed 's/.\{$STRIP_BACK\}$//'"
        fi
    fi
    
    # Strip everything before a delimiter
    if [[ -n "$STRIP_BEFORE" ]]; then
        local escaped_delim="${STRIP_BEFORE//\//\\/}"
        if [[ "$filter" == "cat" ]]; then
            filter="sed 's/.*${escaped_delim}//'"
        else
            filter="$filter | sed 's/.*${escaped_delim}//'"
        fi
    fi
    
    # Strip everything after a delimiter
    if [[ -n "$STRIP_AFTER" ]]; then
        local escaped_delim="${STRIP_AFTER//\//\\/}"
        if [[ "$filter" == "cat" ]]; then
            filter="sed 's/${escaped_delim}.*//'"
        else
            filter="$filter | sed 's/${escaped_delim}.*//'"
...
# Check if any stripping is configured
has_strip_config() {
    [[ -n "$STRIP_PATTERN" ]] || [[ -n "$STRIP_PRESET" ]] || 
    [[ "$STRIP_FRONT" -gt 0 ]] 2>/dev/null || [[ "$STRIP_BACK" -gt 0 ]] 2>/dev/null ||
    [[ -n "$STRIP_BEFORE" ]] || [[ -n "$STRIP_AFTER" ]]
}

# Strip unwanted prefixes from log lines based on STRIP_PATTERN or STRIP_PRESET
strip_log_prefix() {
    local line="$1"
    ...
    case "$STRIP_PRESET" in
        dapr)
            # Strips: "appname: == APP == 2025-11-26 17:21:32,773 - LoggerName - INFO - "
            line=$(echo "$line" | sed -E 's/^[a-zA-Z0-9_-]+: == APP == [0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2},[0-9]+ - [A-Za-z._]+ - (INFO|DEBUG|WARNING|ERROR|CRITICAL) - //')
            ;;
        nx)
            # Strips: "[appname] " prefix
            line=$(echo "$line" | sed -E 's/^\[[a-zA-Z0-9_-]+\] //')
            ;;
        minimal)
            # Strips: timestamp and log level, keeps just the message
            # Handles: "2025-11-26 17:21:32,773 - LoggerName - INFO - message"
            line=$(echo "$line" | sed -E 's/^[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2},[0-9]+ - [A-Za-z._]+ - (INFO|DEBUG|WARNING|ERROR|CRITICAL) - //')
            ;;
    esac
    
    # Apply custom strip pattern if set
    if [[ -n "$STRIP_PATTERN" ]]; then
        line=$(echo "$line" | sed -E "s/$STRIP_PATTERN//")
    fi
    
    # Strip N characters from front
    if [[ "$STRIP_FRONT" -gt 0 ]] 2>/dev/null; then
        line="${line:$STRIP_FRONT}"
    fi
    
    # Strip N characters from back
    if [[ "$STRIP_BACK" -gt 0 ]] 2>/dev/null; then
        local len=${#line}
        local new_len=$((len - STRIP_BACK))
    ...
    # Strip everything before a delimiter
    if [[ -n "$STRIP_BEFORE" ]]; then
        if [[ "$line" == *"$STRIP_BEFORE"* ]]; then
            line="${line#*$STRIP_BEFORE}"
        fi
    fi
    
    # Strip everything after a delimiter
    if [[ -n "$STRIP_AFTER" ]]; then
        if [[ "$line" == *"$STRIP_AFTER"* ]]; then
            line="${line%%$STRIP_AFTER*}"
        fi
    fi
...
colorize_line() {
    ...
    # Apply strip pattern if configured
    if [[ -n "$STRIP_PATTERN" ]] || [[ -n "$STRIP_PRESET" ]] || 
       [[ "$STRIP_FRONT" -gt 0 ]] 2>/dev/null || [[ "$STRIP_BACK" -gt 0 ]] 2>/dev/null ||
       [[ -n "$STRIP_BEFORE" ]] || [[ -n "$STRIP_AFTER" ]]; then
...
${BOLD}COMMANDS${RESET}
    ...
    serve --all           Start ALL apps and capture logs to file
    run <target> <apps>   Run any target for app(s) and capture logs
    run <target> --all    Run target for ALL apps and capture logs
...
${BOLD}OPTIONS${RESET}
    ...
    --strip <preset>      Strip log prefixes (presets: dapr, nx, minimal)
    --strip-pattern <rx>  Custom regex pattern to strip from log lines
    
    -h, --help            Show this help message
    -v, --version         Show version

${BOLD}SERVE & RUN COMMANDS${RESET}
    The serve and run commands support an --all flag to run all apps:
    
        $SCRIPT_NAME serve --all           # Start all apps with logging
        $SCRIPT_NAME serve app1 app2       # Start specific apps
        $SCRIPT_NAME run build --all       # Build all apps
        $SCRIPT_NAME run test app1 app2    # Run tests for specific apps
    
    Each app runs in a separate background process with its own log file.
    Output is streamed to the terminal AND written to logs/nx/<app>.log.
    Press Ctrl+C to stop all running processes.

${BOLD}LOG STRIPPING${RESET}
    Remove verbose prefixes from logs for cleaner output:
    
    Presets:
        dapr      Strips: "app: == APP == timestamp - Logger - LEVEL - "
        nx        Strips: "[appname] " prefix
        minimal   Strips: "timestamp - Logger - LEVEL - "
    
    Character-based stripping:
        STRIP_FRONT=N   Strip N characters from the front of each line
        STRIP_BACK=N    Strip N characters from the back of each line
    
    Examples:
        $SCRIPT_NAME flow --strip dapr       # Clean Dapr logs
        $SCRIPT_NAME api --strip nx          # Clean NX prefixes
        $SCRIPT_NAME flow --strip-pattern '^flow: == APP == '
    
    Configure in .nxlogsrc:
        STRIP_PRESET=dapr
        # Or custom regex:
        STRIP_PATTERN='^[a-z]+: == APP == '
        # Character-based:
        STRIP_FRONT=10   # Remove first 10 characters
        STRIP_BACK=5     # Remove last 5 characters

${BOLD}RUN-MANY COMMAND${RESET}
    Pass arguments directly to nx run-many:
    
        $SCRIPT_NAME run-many -t serve --all
        $SCRIPT_NAME run-many -t build --projects=app1,app2
        $SCRIPT_NAME run-many -t test --parallel=5
...
${BOLD}EXAMPLES${RESET}
    ...
    $SCRIPT_NAME serve --all               # Start all apps with logging
    $SCRIPT_NAME serve api web             # Start specific apps
    $SCRIPT_NAME run build --all           # Build all apps
    $SCRIPT_NAME --stop                    # Stop all running NX processes
    $SCRIPT_NAME --reset                   # Stop processes and clean logs
...
cmd_list_apps() {
    ...
    while IFS= read -r app; do
        ...
        if [[ -f "$log_file" ]]; then
            printf "  ${app_color}●${RESET} %-20s ${GRAY}%6s lines, %6s, %s${RESET}\n" \
                "$app" "$lines" "$size" "$modified"
        else
            printf "  ${GRAY}○ %-20s (no log file)${RESET}\n" "$app"
        fi
    done <<< "$apps"
...
cmd_view() {
    ...
    echo -e "${BOLD}${HEADER_COLOR:-$CYAN}═══ $app ═══${RESET}"
    ...
    if [[ "$follow" == "true" ]]; then
        ...
        echo -e "${BORDER_COLOR:-$GRAY}────────────────────────────────────────────────────────────${RESET}"
...
cmd_view_all() {
    ...
    echo -e "${BOLD}${CYAN}═══ All Apps ═══${RESET}\n"
...
cmd_clean() {
    ...
    if [[ "$force" != "true" ]]; then
        echo -e "${YELLOW}This will delete $count log file(s) in $log_dir (including nested directories)${RESET}"
    ...
    # Delete all log files recursively, including nested directories
    find "$log_dir" -name "*$LOG_EXTENSION" -type f -delete 2>/dev/null
    # Also delete the _combined.log if it exists
    find "$log_dir" -name "_combined.log" -type f -delete 2>/dev/null
...
cmd_stop() {
    echo -e "${BOLD}${CYAN}═══ Stopping NX Processes ═══${RESET}"
...
cmd_rotate() {
    ...
    if [[ "$app" == "all" ]]; then
        ...
        # Find all log files recursively (not just from app discovery)
        if [[ -d "$log_dir" ]]; then
            ...
            done < <(find "$log_dir" -name "*$LOG_EXTENSION" -type f 2>/dev/null)
        fi
        
        if [[ $rotated -gt 0 ]]; then
            log_success "Rotated $rotated log file(s)"
        else
            log_info "No log files needed rotation (threshold: $max_lines lines)"
        fi
    else
        local log_file
        log_file=$(get_log_file "$project_root" "$app")
        if rotate_log_file "$log_file" "$max_lines" "$trim_to"; then
            ...
            log_info "Log file does not need rotation ($current_lines lines, threshold: $max_lines)"
        fi
    fi
}
...
cmd_show_config() {
    ...
    for cf in "${config_files[@]}"; do
        if [[ -f "$cf" ]]; then
            echo -e "  ${GREEN}✓${RESET} $cf"
        else
            echo -e "  ${GRAY}○ $cf${RESET}"
        fi
    done
    echo ""
}
...
cmd_demo() {
    ...
    echo -e "${BOLD}${CYAN}═══ Generating Demo Logs ═══${RESET}\n"
    ...
    for app in "${demo_apps[@]}"; do
        ...
        stats=$(get_log_stats "$log_file")
        IFS='|' read -r lines size modified <<< "$stats"
        echo -e "  ${GREEN}✓${RESET} Generated $lines lines ($size)"
    done
    ...
    
    {
        ...
        if [[ "$format" == "json" ]]; then
            echo "$content" | while IFS= read -r line; do
                [[ -n "$line" ]] && format_line_json "$line" "$app" "$pretty"
            done
        else
            echo "$content"
        fi
...
process_multiplexed_logs() {
    ...
    local do_strip=false
    if has_strip_config; then
        do_strip=true
    fi
    ...
    while IFS= read -r line; do
        ...
        if [[ -n "$app_name" ]]; then
            ...
            # Apply stripping before writing to file
            local stripped_line="$line"
            if [[ "$do_strip" == "true" ]]; then
                stripped_line=$(strip_log_prefix "$line")
            fi
            echo "$stripped_line" >> "$log_file"
            
            # Format for display (strip_log_prefix is called inside format_follow_line -> colorize_line)
            ...
            echo "$stripped_line" >> "$log_dir/_combined.log"
...
cmd_run_many() {
    ...
    echo -e "${BOLD}${CYAN}═══ Running run-many ═══${RESET}"
...
cmd_run() {
    ...
    echo -e "${BOLD}${CYAN}═══ Running target '$target' for ${#apps[@]} app(s) ═══${RESET}"
    ...
    # Build strip filter for log files if configured
    local strip_filter="cat"
    if has_strip_config; then
        strip_filter=$(build_strip_filter)
        echo -e "${GRAY}Log stripping enabled: $strip_filter${RESET}"
    fi
    echo ""
    ...
    echo ""
    echo -e "${BOLD}✅ All apps starting... Output streaming below (Ctrl+C to stop all)${RESET}"
    echo -e "${BORDER_COLOR:-$GRAY}═══════════════════════════════════════════════════════════════════${RESET}"
...
cmd_daemon() {
    ...
    echo -e "${BOLD}${CYAN}═══ NX Daemon Logs ═══${RESET}"
...
cmd_cache() {
    ...
    echo -e "${BOLD}${CYAN}═══ NX Cache/Task Logs ═══${RESET}"
    ...
    case "$cmd" in
        ...
        *)
            ...
            if [[ -n "$export_file" ]]; then
                ...
                for app in "${apps[@]}"; do
                    cmd_export "$project_root" "$app" "$export_file" "$lines" "$search" "$errors_only" "$since" "$until" "$format" "$pretty_json"
                done
                ...
                for app in "${apps[@]}"; do
                    if [[ "$json_output" == "true" ]]; then
                        cmd_view_json "$project_root" "$app" "$lines" "$search" "$errors_only" "$since" "$until" "$pretty_json"
                    else
